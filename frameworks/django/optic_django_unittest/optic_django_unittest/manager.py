...
class HttpInteractionManager(BasicOpticManager):
    ...
    def wrap_post_tear_down(cls, tear_down):
        def wrapped(self, *args, **kwargs):
            if not hasattr(cls, 'test_result_container') or not OpticDjangoAppConfig.enabled():
    ...
    def patch_result(cls):
        TestResult.addSuccess = cls.wrap_result_handler(
            TestResult.addSuccess, True
        )
        TestResult.addFailure = cls.wrap_result_handler(
            TestResult.addFailure, False
        )
        TestResult.addError = cls.wrap_result_handler(
            TestResult.addError, False
        )
